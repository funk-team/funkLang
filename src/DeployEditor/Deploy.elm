...
deployIfValid : Url.Url -> Model -> Cmd Msg
deployIfValid url model =
    let
        siteSlug =
            model.siteName
                |> Slug.generate
    ...
    in
    case Maybe.map2 Tuple.pair (Route.getProjectData url) siteSlug of
        Just info ->
            deploy info
...
update : Msg -> Url.Url -> Model -> ( Model, Cmd Msg )
update msg url model =
    case msg of
        ...
        Deploy ->
            ( { model | status = CollectingAssets }, deployIfValid url model )
