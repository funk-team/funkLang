...
export const getProjectRootDirectory = projectMeta => `/${projectMeta.projectId}`;
...
export const use = addons => app => {
    ...
    const simpleCheckout = app => async projectMeta => {
        ...
        app.ports.repoInitialized.send(await Persistence.readSpec(projectRoot));
    }

    app.ports.save.subscribe(save);
    app.ports.checkout.subscribe(addons ? addons.checkout(app) : simpleCheckout(app));
    ...
    window.addEventListener("storage", ev => {
        ...
        if (app.ports.previewSpecUpdated) {
            F.fork(err =>
                ...
                app.ports.previewSpecUpdated.send([
                    projectMeta,
                    decodeBuffer(buffer)
                ])
            )(
...
export const readSpec = async repoPath => {
    ...
    try {
        const fileContents = decodeBuffer(
            await fs.promises.readFile(funkFilePath)
        );
        return fileContents;
    } catch (e) {
        return null;
